...
const hashPassword = async (password) => {
  ...
  const pwdStr = Buffer.isBuffer(password) ? password.toString() : password;
  const salt = await bcrypt.genSalt(10);
  const hash = await bcrypt.hash(pwdStr, salt);
  return Buffer.from(hash);
};

const createRandomHex = (len) => {
